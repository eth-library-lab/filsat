...
async function taskGenerate(config, flags) {
    ...
    const outDir = path.join(baseDir, srcDir, 'components', dir, componentName);
 ...
 */
const writeFileByExtension = async (path$1, name, extension, withCss) => {
    const outFile = path.join(path$1, `${name}.${extension}`);
 ...
 */
const getSpecTestBoilerplate = (tagName) => `import { ${toPascalCase(tagName)} } from './${tagName}';
